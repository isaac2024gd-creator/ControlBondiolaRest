...
import { Clock, Package, Receipt, ChevronRight } from "lucide-react";
...
const MODULOS = [
  ...
  },
  {
    ...
    subtitulo: "Conteo diario de perecederos, limpieza y pendientes",
    color: "#1F5C4D",
    bg: "#F7F3EC",
  },
  {
  ...
  if (modulo === "checador") {
    return (
      <div className="relative">
        <BotonVolver onClick={volverAlInicio} oscuro />
        <RelojChecador />
      </div>
    );
  }
...
function BotonVolver({ onClick, oscuro }) {
  return (
    <button
      onClick={onClick}
      className="fixed top-3 left-3 z-[60] flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold"
      style={{
        background: oscuro ? "#00000066" : "#FFFDF9CC",
        color: oscuro ? "#F7F3EA" : "#221F1A",
