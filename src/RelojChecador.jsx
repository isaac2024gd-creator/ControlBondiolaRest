      ...
      <div className="rounded-sm p-4" style={{ background: paper }}>
        ...
        <div className="mt-4 p-3 rounded-sm" style={{ background: brass + "11" }}>
          <p className="text-[10px] mb-3" style={{ color: ink + "88" }}>
            <strong>Generar recibo:</strong> Crea la nómina imprimible. El botón de guardar configuración arriba es para guardar los datos de pago de este empleado (tarifa, periodo, etc.).
          </p>
        </div>
...
export default function RelojChecador() {
  ...
  const [confirmGeneratePayroll, setConfirmGeneratePayroll] = useState(null); // { employee, worked, totals, draft } | null
  ...
  function handleGeneratePayroll(worked, totals) {
    ...
    setConfirmGeneratePayroll({ employee: emp, worked, totals, draft: payrollDraft });
  }

  function confirmAndGeneratePayroll() {
    if (!confirmGeneratePayroll) return;
    const { employee: emp, worked, totals, draft } = confirmGeneratePayroll;
    ...
    setPayrollPrintView({ employee: emp, draft: draft, worked, totals });
  ...
  function requestUnlock(target) {
    if (pin === undefined) {
      // Todavía no termina de cargar el PIN guardado desde Supabase.
        ...
        >
          ...
          <div style={{ marginTop: "0.8rem", padding: "0.6rem 0.75rem", backgroundColor: "#f5f5f5", borderRadius: 3, border: "1px solid #ddd" }}>
            ...
            {pDraft.tieneSeguroSocial ? (
              ...
              <p style={{ fontSize: "0.66rem", color: "#666", margin: 0 }}>
                El trabajador no cuenta con afiliación al Seguro Social en nómina.
              </p>
            )}
          </div>
