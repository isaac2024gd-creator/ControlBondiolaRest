...
// ---------- marca de agua con fecha y hora (solo fotos de checado, no el logo) ----------
function drawTimestampWatermark(ctx, w, h) {
  ...
  const fontSize = Math.max(11, Math.round(w * 0.055));
  ...
  ctx.textAlign = "left";
  ctx.fillText(stamp, paddingX, h - barHeight / 2 + 1);
}

function compressImage(file, maxWidth = 480, quality = 0.6, { watermark = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        if (watermark) drawTimestampWatermark(ctx, w, h);
...
function computeWorkedInRange(employeeId, startKey, endKey, recordsByDate) {
  ...
  let diaEnCurso = false; // true si hoy ya tiene entrada pero todavía no tiene salida
  ...
  dates.forEach((dateKey) => {
    ...
    if (salida) {
      ...
      // Día en curso: ya hay entrada de hoy pero todavía no hay salida (el empleado sigue
      // trabajando). El día ya se contó arriba (totalDias); si además cobra por hora, se
      // calculan las horas transcurridas desde su entrada hasta este momento, para que la
      // nómina de hoy no se quede en $0 solo por no haber salida todavía. Si se vuelve a
      // generar el recibo después de que marque salida, el número se ajusta solo con la
      // hora real.
      diaEnCurso = true;
      const h = hoursBetween(entrada.time, new Date().toISOString());
      if (h) totalHoras += h;
    }
  });
...
// ---------- días feriados (catálogo automático + cálculo por ley) ----------
// Devuelve la fecha del enésimo día de la semana (weekday: 0=domingo..6=sábado) de un mes,
// para los feriados "movibles" de la LFT (ej. primer lunes de febrero).
...
// catálogo por default: los 7 feriados obligatorios "de todos los años" que marca el
// Art. 74 de la Ley Federal del Trabajo (no incluye el 1° de diciembre de transmisión del
// Poder Ejecutivo, que solo aplica cada 6 años — ese Isaac puede agregarlo a mano si toca).
function defaultFeriadosCatalogo(years) {
  const catalogo = [];
  years.forEach((year) => {
    const add = (date, nombre) => catalogo.push({ fecha: localDateKey(date), nombre, esLey: true, activo: true });
    add(new Date(year, 0, 1), "Año nuevo");
    add(nthWeekdayOfMonth(year, 2, 1, 1), "Día de la Constitución (1er lunes de feb.)");
    add(nthWeekdayOfMonth(year, 3, 1, 3), "Natalicio de Benito Juárez (3er lunes de mar.)");
    add(new Date(year, 4, 1), "Día del Trabajo");
    add(new Date(year, 8, 16), "Día de la Independencia");
    add(nthWeekdayOfMonth(year, 11, 1, 3), "Día de la Revolución (3er lunes de nov.)");
    add(new Date(year, 11, 25), "Navidad");
  });
  return catalogo.sort((a, b) => a.fecha.localeCompare(b.fecha));
}

// fechas del catálogo (activas) que caen dentro del periodo de nómina
function getFeriadosEnRango(startKey, endKey, catalogo) {
  return (catalogo || []).filter((f) => f.activo && f.fecha >= startKey && f.fecha <= endKey);
}

// ¿el empleado checó entrada o salida ese día específico? (mismo criterio que
// getNonWorkedDaysInRange, pero para una sola fecha puntual del catálogo de feriados)
function empleadoTrabajoEseDia(employeeId, dateKey, recordsByDate) {
  const dayRecords = recordsByDate[dateKey] || [];
  return dayRecords.some((r) => r.employeeId === employeeId && (r.type === "entrada" || r.type === "salida"));
}

// monto que corresponde a UN día feriado del catálogo, para un empleado:
// - si hay un monto personalizado activado para esa fecha (switch por instancia), se usa ese
// - feriado NO trabajado: se paga el día completo aparte (no está en "Pago base", porque no
//   se checó) — Art. 74 LFT.
// - feriado SÍ trabajado: el día ya se pagó una vez dentro de "Pago base" (si checó, cuenta
//   en worked.totalDias/totalHoras) — aquí solo se agrega el DOBLE extra que exige el
//   Art. 75 LFT, para que sumado al pago base dé el triple correcto.
function computeFeriadoMonto(employee, fecha, draft, worked, recordsByDate) {
  const override = draft.feriadosOverrides?.[fecha];
  if (override?.personalizado) return Number(override.monto) || 0;
  const trabajado = empleadoTrabajoEseDia(employee.id, fecha, recordsByDate);
  const diaPay = computeVacacionDayPay(employee, fecha, draft, worked);
  return trabajado ? diaPay * 2 : diaPay;
}

// ---------- ISR e IMSS reales (Nómina Formal) ----------
// Constantes fiscales 2026 — actualizar cada año (UMA/salario mínimo en enero, tablas ISR
// con el DOF; las tasas IMSS rara vez cambian). Detalle y fuentes completas en los
// documentos "simulador-nomina-real.md" y "recibo-nomina-formal.md" del proyecto de
// comercialización — validar con contador antes de usarse con nómina real.
const UMA_DIARIA_2026 = 117.31;

// Tarifas ISR por periodicidad de pago (límite inferior, límite superior, cuota fija,
// % sobre excedente). Mismo % marginal en las tres tablas, solo cambia la escala del periodo.
const TABLA_ISR_MENSUAL_2026 = [
  { limInf: 0.01, limSup: 844.59, cuota: 0, pct: 1.92 },
  { limInf: 844.6, limSup: 7168.51, cuota: 16.22, pct: 6.4 },
  { limInf: 7168.52, limSup: 12598.02, cuota: 420.95, pct: 10.88 },
  { limInf: 12598.03, limSup: 14644.64, cuota: 1011.68, pct: 16.0 },
  ...
  { limInf: 55736.69, limSup: 106410.5, cuota: 10457.09, pct: 30.0 },
  { limInf: 106410.51, limSup: 141880.66, cuota: 25659.23, pct: 32.0 },
  { limInf: 141880.67, limSup: 425641.99, cuota: 37009.69, pct: 34.0 },
  { limInf: 425642.0, limSup: Infinity, cuota: 133488.54, pct: 35.0 },
];
const TABLA_ISR_QUINCENAL_2026 = [
  { limInf: 0.01, limSup: 422.3, cuota: 0, pct: 1.92 },
  { limInf: 422.3, limSup: 3584.26, cuota: 8.11, pct: 6.4 },
  { limInf: 3584.26, limSup: 6299.01, cuota: 210.48, pct: 10.88 },
  { limInf: 6299.02, limSup: 7322.32, cuota: 505.84, pct: 16.0 },
  { limInf: 7322.33, limSup: 8766.82, cuota: 669.57, pct: 17.92 },
  { limInf: 8766.83, limSup: 17681.42, cuota: 928.42, pct: 21.36 },
  { limInf: 17681.42, limSup: 27868.34, cuota: 2832.58, pct: 23.52 },
  { limInf: 27868.35, limSup: 53205.25, cuota: 5228.55, pct: 30.0 },
  { limInf: 53205.26, limSup: 70940.33, cuota: 12829.62, pct: 32.0 },
  { limInf: 70940.34, limSup: 212821.0, cuota: 18504.85, pct: 34.0 },
  { limInf: 212821.0, limSup: Infinity, cuota: 66744.27, pct: 35.0 },
];
const TABLA_ISR_SEMANAL_2026 = [
  { limInf: 0.01, limSup: 194.46, cuota: 0, pct: 1.92 },
  { limInf: 194.47, limSup: 1650.67, cuota: 3.71, pct: 6.4 },
  { limInf: 1650.68, limSup: 2900.87, cuota: 96.95, pct: 10.88 },
  { limInf: 2900.88, limSup: 3372.11, cuota: 232.96, pct: 16.0 },
  ...
  { limInf: 12834.09, limSup: 24502.45, cuota: 2407.86, pct: 30.0 },
  { limInf: 24502.46, limSup: 32669.91, cuota: 5908.35, pct: 32.0 },
  { limInf: 32669.92, limSup: 98009.66, cuota: 8521.94, pct: 34.0 },
  { limInf: 98009.67, limSup: Infinity, cuota: 30737.49, pct: 35.0 },
];

// ISR retenido real, según la tabla del SAT que corresponda a la periodicidad de pago del
// empleado. Aproximado: no resta el subsidio para el empleo (pendiente de fase 2).
function calcularISR(gravable, payPeriod) {
  const tabla =
    payPeriod === "semanal" ? TABLA_ISR_SEMANAL_2026 : payPeriod === "mensual" ? TABLA_ISR_MENSUAL_2026 : TABLA_ISR_QUINCENAL_2026;
  const monto = Number(gravable) || 0;
  const renglon = tabla.find((r) => monto >= r.limInf && monto <= r.limSup) || tabla[tabla.length - 1];
  return renglon.cuota + (monto - renglon.limInf) * (renglon.pct / 100);
}

// IMSS descontado al trabajador (obrero): 2.375% fijo sobre el SBC diario (suma de
// Enfermedad y Maternidad-prestaciones en dinero 0.25% + Gastos médicos de pensionados
// 0.375% + Invalidez y Vida 0.625% + Cesantía y Vejez 1.125%) más 0.40% sobre la parte del
// SBC diario que exceda 3 UMAs (excedente de Enfermedad y Maternidad). SBC simplificado =
// salario diario capturado, sin integrar aguinaldo/prima vacacional (aproximado — ver
// "recibo-nomina-formal.md"). Riesgo de Trabajo (patronal, variable por negocio) no se
// incluye — no aplica al trabajador de todos modos.
function calcularIMSSObrero(sbcDiario, diasCotizados) {
  const sbc = Number(sbcDiario) || 0;
  const dias = Number(diasCotizados) || 0;
  const excedente = Math.max(sbc - UMA_DIARIA_2026 * 3, 0);
  const porDia = sbc * 0.02375 + excedente * 0.004;
  return porDia * dias;
}

function computePayrollTotals(draft, worked, bonoUnits, propinaMonto, employee, feriadosMonto) {
  ...
  const vacaciones = draft.enableVacaciones
    ...
    : 0;
  // permisos especiales / ajustes extraordinarios: días que NO se contabilizan a partir de
  // checadas reales (el empleado olvidó checar, o se le concedió un permiso especial), pero
  // que sí se pagan como una jornada completa — usa la misma fórmula de "pago de un día" que
  // vacaciones (jornada completa si cobra por día; horario asignado o promedio si cobra por
  // hora), solo que aquí el motivo es distinto (olvido/permiso, no vacaciones).
  const permisos = draft.enablePermisos
    ? (draft.permisosFechas || []).reduce((s, p) => s + computeVacacionDayPay(employee, p.fecha, draft, worked), 0)
    : 0;
  const feriados = draft.enableFeriados ? feriadosMonto || 0 : 0;
  ...
  const consumo = draft.enableConsumo
    ...
    : 0;
  const bruto = base + bono + propina + vacaciones + permisos + feriados + horasExtra;
  // Nómina Formal (tieneSeguroSocial=true): ISR + IMSS obrero reales, calculados con las
  // tablas vigentes — reemplaza el antiguo "descuento de ley (%)" simplificado. Nómina
  // Simplificada (default): sin deducciones, igual que siempre.
  const sbcDiario = draft.rateType === "hora" ? (worked.totalDias > 0 ? base / worked.totalDias : 0) : draft.rateAmount;
  const isrDeduccion = draft.tieneSeguroSocial ? calcularISR(bruto, draft.payPeriod) : 0;
  const imssDeduccion = draft.tieneSeguroSocial ? calcularIMSSObrero(sbcDiario, worked.totalDias) : 0;
  const neto = bruto - isrDeduccion - imssDeduccion - descuentos - consumo;
  return { base, bono, propina, vacaciones, permisos, feriados, horasExtra, descuentos, consumo, bruto, isrDeduccion, imssDeduccion, neto };
}
...
function calcularRepartoPropinas(startKey, endKey, monto, employees, recordsByDate, externosPorDia) {
  const candidatos = employees
    .map((emp) => {
      const w = computeWorkedInRange(emp.id, startKey, endKey, recordsByDate);
      // motivo(s) de la(s) corrección(es) manual(es) en el rango, para mostrarlos como
      // comentario junto al reparto de esta persona (no solo el aviso genérico "Corregido").
      const motivosCorreccion = Object.keys(recordsByDate)
        .filter((d) => d >= startKey && d <= endKey)
        .flatMap((d) =>
          (recordsByDate[d] || [])
            .filter((r) => r.employeeId === emp.id && r.type === "entrada" && r.correccionManual)
            .map((r) => ({
              fecha: d,
              motivo: r.correccionManual.motivo || "",
              penalizacion: !!r.correccionManual.penalizacion,
            }))
        );
      return {
        employeeId: emp.id,
        ...
        tieneCorreccion: motivosCorreccion.length > 0,
        motivosCorreccion,
        tipo: "empleado",
      };
    })
...
// ---------- fotos del Checador en Supabase Storage (con marca de agua) ----------
// A diferencia del logo (arriba), las fotos de entrada/salida SÍ se suben a Supabase:
// así se ven desde cualquier dispositivo (celular del Dueño, otra tablet, etc.) y no
// se pierden si se borra la caché del navegador del equipo que las tomó.
const PHOTO_BUCKET = "fotos-checador";

function photoStoragePath(recordId) {
  return `checador/${recordId}.jpg`;
}
...
async function uploadPhotoRemote(path, dataUrl) {
...
// switch compacto para usar EN LÍNEA junto a otro contenido (a diferencia de ToggleRow,
// que ocupa toda la fila) — se usa para el "monto personalizado" de cada feriado y para
// activar/desactivar cada fecha del catálogo.
function MiniToggle({ label, checked, onChange, colors }) {
  const { ink, sage, steel } = colors;
  return (
    <button onClick={() => onChange(!checked)} className="flex items-center gap-1.5 flex-shrink-0">
      {label && (
      ...
      <span
        ...
        style={{ width: 26, height: 15, background: checked ? sage : steel + "55", position: "relative", transition: "background 0.15s" }}
      >
        <span
          className="rounded-full absolute"
          style={{ width: 11, height: 11, top: 2, left: checked ? 13 : 2, background: "#fff", transition: "left 0.15s" }}
        />
      </span>
    </button>
  );
...
function PayrollPanel({
  ...
  // feriados: se detectan solos cruzando el catálogo con las fechas del periodo, y cada
  // uno se resuelve por ley (o por el monto personalizado de esa fecha, si se activó) —
  // se congela igual que vacacionesDetalle, para que reimprimir el recibo no cambie si
  // luego se edita el catálogo o la bitácora de ese día.
  const feriadosEnPeriodo = draft.enableFeriados
    ? getFeriadosEnRango(draft.periodStart, draft.periodEnd, feriadosCatalogo)
    : [];
  const feriadosDetalle = feriadosEnPeriodo.map((f) => {
    const override = draft.feriadosOverrides?.[f.fecha];
    const trabajado = empleadoTrabajoEseDia(employee.id, f.fecha, recordsByDate);
    const monto = computeFeriadoMonto(employee, f.fecha, draft, worked, recordsByDate);
    return {
      fecha: f.fecha,
      nombre: f.nombre,
      trabajado,
      monto,
      personalizado: !!override?.personalizado,
      motivo: override?.personalizado ? override?.motivo || "" : "",
    };
  });
  const feriadosMonto = feriadosDetalle.reduce((s, f) => s + f.monto, 0);
  const totals = computePayrollTotals(draft, worked, bonoUnits, propinaSeSuma ? propinaAuto : 0, employee, feriadosMonto);
  const bonoLabel =
  ...
  // días sin checar del periodo: se comparten entre Vacaciones y Permisos especiales, cada
  // uno excluyendo los días que el otro ya marcó, para que un mismo día no se pague doble.
  const diasNoTrabajados =
    draft.enableVacaciones || draft.enablePermisos
      ? getNonWorkedDaysInRange(employee.id, draft.periodStart, draft.periodEnd, recordsByDate)
      : [];
  const diasNoTrabajadosParaVacaciones = diasNoTrabajados.filter(
    (d) => !(draft.permisosFechas || []).some((p) => p.fecha === d.key)
  );
  const diasNoTrabajadosParaPermisos = diasNoTrabajados.filter((d) => !(draft.vacacionesFechas || []).includes(d.key));
  ...
  const vacacionesDetalle = draft.enableVacaciones
    ...
    : [];
  // detalle de permisos especiales / ajustes extraordinarios — mismo criterio de congelado
  // que vacaciones, pero guarda además el tipo (olvido de checada o permiso autorizado) y el
  // motivo, para que el recibo quede documentado.
  const permisosDetalle = draft.enablePermisos
    ? (draft.permisosFechas || []).map((p) => ({
        fecha: p.fecha,
        tipo: p.tipo || "olvido",
        motivo: p.motivo || "",
        monto: computeVacacionDayPay(employee, p.fecha, draft, worked),
      }))
    : [];
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col gap-4 pb-4">
      ...
      <div className="rounded-sm p-4" style={{ background: paper }}>
        ...
        {worked.diaEnCurso && (
          <p className="text-[10px] mt-1 font-bold" style={{ color: brass }}>
            ⏱️ El día de hoy ya cuenta porque el empleado marcó entrada, pero todavía no marca salida — las horas de
            hoy son provisionales y se ajustan solas si generas el recibo de nuevo después de su salida.
          </p>
        )}
      </div>
      ...
      <div className="rounded-sm p-4 flex flex-col gap-2" style={{ background: paper }}>
        ...
        {draft.enableVacaciones && (
          <div className="pl-1">
            {diasNoTrabajadosParaVacaciones.length === 0 ? (
              ...
              <>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {diasNoTrabajadosParaVacaciones.map((d) => {
        ...
        <ToggleRow
          label="Permiso especial / ajuste extraordinario"
          checked={draft.enablePermisos}
          onChange={(v) => onChange({ enablePermisos: v })}
          colors={colors}
        />
        {draft.enablePermisos && (
          <div className="pl-1">
            <p className="text-[10px] mb-2" style={{ color: ink + "66" }}>
              Para cuando el empleado olvidó checar, o se le concedió un permiso especial: el día
              no se cuenta a partir de sus checadas, pero sí se le paga como jornada completa.
            </p>
            {diasNoTrabajadosParaPermisos.length === 0 ? (
              <p className="text-[11px]" style={{ color: ink + "66" }}>
                No hay días sin checar dentro de este periodo — no hay nada que marcar con permiso
                especial.
              </p>
            ) : (
              <>
                <p className="text-[11px] mb-1.5" style={{ color: ink + "88" }}>
                  ¿Cuál de estos días sin checar fue por olvido o permiso especial?
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {diasNoTrabajadosParaPermisos.map((d) => {
                    const marcado = (draft.permisosFechas || []).some((p) => p.fecha === d.key);
                    return (
                      <button
                        key={d.key}
                        onClick={() => {
                          const actuales = draft.permisosFechas || [];
                          onChange({
                            permisosFechas: marcado
                              ? actuales.filter((p) => p.fecha !== d.key)
                              : [...actuales, { fecha: d.key, tipo: "olvido", motivo: "" }],
                          });
                        }}
                        className="px-2.5 py-1.5 rounded-sm text-[11px] font-bold"
                        style={{
                          background: marcado ? brass : "transparent",
                          color: marcado ? ink : ink,
                          border: `1px solid ${marcado ? brass : ink + "33"}`,
                        }}
                      >
                        {d.weekday} {d.key.slice(8, 10)}
                      </button>
            ...
            {(draft.permisosFechas || []).length > 0 && (
              <div className="flex flex-col gap-2 mt-2">
                {draft.permisosFechas.map((p, idx) => (
                  <div key={p.fecha} className="rounded-sm p-2" style={{ background: "#fff", border: `1px solid ${ink}22` }}>
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      <span className="text-[11px] font-bold" style={{ color: ink }}>
                        {formatDateLabel(p.fecha, localDateKey())} · {p.fecha}
                      </span>
                      <button
                        onClick={() => {
                          const actuales = draft.permisosFechas || [];
                          onChange({ permisosFechas: actuales.filter((_, i) => i !== idx) });
                        }}
                        style={{ color: ink + "55" }}
                      >
                        <Trash2 size={13} />
                    ...
                    <div className="flex gap-1.5 mb-1.5">
                      {[
                        { value: "olvido", label: "Olvidó checar" },
                        ...
                        <button
                          ...
                          onClick={() => {
                            const actuales = [...(draft.permisosFechas || [])];
                            actuales[idx] = { ...actuales[idx], tipo: opt.value };
                            onChange({ permisosFechas: actuales });
                          }}
                          className="px-2 py-1 rounded-sm text-[10px] font-bold"
                          style={{
                            background: p.tipo === opt.value ? brass : "transparent",
                            color: ink,
                            border: `1px solid ${p.tipo === opt.value ? brass : ink + "33"}`,
                          }}
                        >
                          {opt.label}
                        </button>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const actuales = [...(draft.permisosFechas || [])];
                        actuales[idx] = { ...actuales[idx], motivo: e.target.value };
                        onChange({ permisosFechas: actuales });
                      }}
                      placeholder="Motivo (opcional) — ej. cita médica, trámite personal…"
                      className="w-full px-2 py-1.5 rounded-sm text-[11px] outline-none"
                      style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
                    />
                  </div>
                ))}
                <p className="text-[10px]" style={{ color: brass }}>
                  {draft.permisosFechas.length} día(s) marcado(s) con permiso especial = {formatMoney(totals.permisos)}
                  {draft.rateType === "hora" && (
                    <> (calculado según el horario asignado de cada día, no la tarifa por hora tal cual)</>
                  )}
                  .
        ...
        <ToggleRow
          label="Días feriados"
          checked={draft.enableFeriados}
          onChange={(v) => onChange({ enableFeriados: v })}
          colors={colors}
        />
        {draft.enableFeriados && (
          <div className="pl-1 flex flex-col gap-2">
            {feriadosDetalle.length === 0 ? (
              <p className="text-[11px]" style={{ color: ink + "66" }}>
                No hay ningún día feriado del catálogo dentro de este periodo.
              </p>
            ) : (
              feriadosDetalle.map((f) => {
                const override = draft.feriadosOverrides?.[f.fecha] || {};
                return (
                  <div
                    key={f.fecha}
                    className="rounded-sm p-2.5"
                    style={{ background: f.trabajado ? brass + "14" : sage + "14" }}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="text-[10px]" style={{ color: ink + "77" }}>
                          {formatFechaLargaEs(f.fecha)} · {f.trabajado ? "Se trabajó ese día" : "No se trabajó ese día"}
                        </div>
                      </div>
                      <div className="text-sm font-bold flex-shrink-0" style={{ color: sage }}>
                        {formatMoney(f.monto)}
                      </div>
                    </div>
                    <div className="flex items-center justify-between gap-2 mt-1.5">
                      <span className="text-[10px] flex-1" style={{ color: ink + "88" }}>
                        {f.trabajado
                          ? "Por ley: pago base + doble extra (triple en total)."
                          : "Por ley: se paga el día completo aunque no se trabajó."}
                      </span>
                      <MiniToggle
                        label="Monto personalizado"
                        checked={!!override.personalizado}
                        onChange={(v) =>
                          onChange({
                            feriadosOverrides: {
                              ...(draft.feriadosOverrides || {}),
                              [f.fecha]: { ...override, personalizado: v },
                            },
                          })
                        }
                        colors={colors}
                    ...
                    {override.personalizado && (
                      <div className="mt-1.5 flex flex-col gap-1.5">
                        <input
                          value={override.monto ?? ""}
                          onChange={(e) => {
                            const val = e.target.value.replace(/[^0-9.]/g, "");
                            onChange({
                              feriadosOverrides: {
                                ...(draft.feriadosOverrides || {}),
                                [f.fecha]: { ...override, personalizado: true, monto: val },
                              },
                            });
                          }}
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            onChange({
                              feriadosOverrides: {
                                ...(draft.feriadosOverrides || {}),
                                [f.fecha]: { ...override, personalizado: true, motivo: e.target.value },
                              },
                            })
                          }
                          placeholder="Motivo del monto personalizado (obligatorio)"
                          className="px-2 py-1.5 rounded-sm text-xs outline-none"
                          style={{ border: `1px solid ${!override.motivo ? paprika : ink}33`, background: "#fff", color: ink }}
                        />
                        {!override.motivo && (
                          <span className="text-[9px]" style={{ color: paprika }}>
                            ⚠️ escribe el motivo antes de generar el recibo
                          </span>
                        )}
                      </div>
                    )}
            ...
            {feriadosDetalle.length > 0 && (
              <p className="text-[10px]" style={{ color: sage }}>
                Total feriados de este periodo = {formatMoney(feriadosMonto)}
              </p>
            )}
          </div>
        )}
        ...
        <ToggleRow
          label="Nómina Formal (con Seguro Social)"
        ...
        {draft.tieneSeguroSocial ? (
          <div className="pl-1 flex flex-col gap-2">
            <p className="text-[10px] flex items-start gap-1" style={{ color: brass }}>
              <Info size={11} className="flex-shrink-0 mt-0.5" />
              El recibo calculará y descontará ISR e IMSS reales de este trabajador (tablas del SAT/IMSS
              vigentes, según su periodicidad de pago). Es un cálculo aproximado — antes de usarlo con nómina
              de verdad, valídalo con tu contador.
            </p>
            <div className="flex items-center gap-2">
              <span className="text-[11px]" style={{ color: ink + "88" }}>
            ...
            <p className="text-[10px]" style={{ color: ink + "66" }}>
              Estos datos se guardan en la configuración de {employee.name} — sus próximos recibos serán
              Nómina Formal, con Deducciones (ISR + IMSS) reales.
            </p>
          </div>
        ) : (
          <p className="text-[10px] pl-1" style={{ color: ink + "66" }}>
            Nómina Simplificada: el recibo de {employee.name} sigue igual que hoy, sin ISR ni IMSS descontados.
          </p>
        )}
      </div>
      ...
      <div className="rounded-sm p-4" style={{ background: paper }}>
        ...
        <div className="flex flex-col gap-1 text-xs" style={{ color: ink }}>
          ...
          {draft.enablePermisos && (draft.permisosFechas || []).length > 0 && (
            <div className="flex justify-between">
              <span>Permiso especial ({draft.permisosFechas.length})</span>
              <span>+{formatMoney(totals.permisos)}</span>
            </div>
          )}
          {draft.enableFeriados && feriadosDetalle.length > 0 && (
            <div className="flex justify-between">
              <span>Días feriados ({feriadosDetalle.length})</span>
              <span>+{formatMoney(totals.feriados)}</span>
            </div>
          )}
          {mostrarLineaHorasExtra && (
            <div className="flex justify-between">
          ...
          {draft.tieneSeguroSocial && (
            <>
              <div className="flex justify-between" style={{ color: paprika }}>
                ...
                <span>-{formatMoney(totals.isrDeduccion)}</span>
              </div>
              <div className="flex justify-between" style={{ color: paprika }}>
                <span>IMSS (trabajador)</span>
                <span>-{formatMoney(totals.imssDeduccion)}</span>
              </div>
            </>
          )}
          {draft.enableDescuentos && (
        ...
        {feriadosDetalle.some((f) => f.personalizado && !f.motivo) && (
          <p className="text-[10px] mb-2 flex items-start gap-1" style={{ color: paprika }}>
            <AlertTriangle size={11} className="flex-shrink-0 mt-0.5" />
            Falta el motivo de al menos un monto personalizado en Días feriados.
          </p>
        )}
        <button
          onClick={() => onGenerate(worked, totals, propinaSeSuma, vacacionesDetalle, feriadosDetalle, permisosDetalle)}
          disabled={totals.neto < 0 || feriadosDetalle.some((f) => f.personalizado && !f.motivo)}
...
export default function RelojChecador() {
  ...
  // ---------- reloj en tiempo real (referencia visual para el Checador) ----------
  ...
  // ---------- caché en memoria de fotos legado leídas desde IndexedDB ----------
  // Solo aplica a registros VIEJOS (de antes de subir las fotos a Supabase Storage),
  // cuya foto quedó únicamente en el dispositivo que la tomó. Los registros nuevos ya
  // traen `photoUrl` y no necesitan esto.
  ...
  const [photoViewer, setPhotoViewer] = useState(null); // {url, employeeName, type, time} — ver foto en grande desde Bitácora
  ...
  function handleGeneratePayroll(worked, totals, propinaSeSuma, vacacionesDetalle, feriadosDetalle, permisosDetalle) {
  ...
  async function confirmAndGeneratePayroll() {
    ...
    const { employee: emp, worked, totals, draft, propinaSeSuma, vacacionesDetalle, feriadosDetalle, permisosDetalle } =
    ...
    setPayrollPrintView({ employee: emp, draft: draft, worked, totals, propinaSeSuma, vacacionesDetalle, feriadosDetalle, permisosDetalle });
  ...
  function reimprimirPayrollRun(run) {
    ...
    setPayrollPrintView({
      ...
      feriadosDetalle: run.feriadosDetalle,
      permisosDetalle: run.permisosDetalle,
    });
  }
  ...
  // ---------- catálogo de días feriados (para Nómina) ----------
  // Visible para cualquiera sin PIN (igual que propinasConfig); solo EDITARLO pide clave.
  // Se precarga con los feriados obligatorios de la LFT de un rango de años, y cada vez
  // que se agrega un año nuevo al rango se fusiona con lo ya guardado sin perder lo que
  // Isaac haya cambiado (activo/inactivo, fechas propias que haya agregado).
  const [feriadosCatalogo, setFeriadosCatalogo] = useState([]);
  const [showFeriadosCatalogoEdit, setShowFeriadosCatalogoEdit] = useState(false);
  const [feriadosCatalogoDraft, setFeriadosCatalogoDraft] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      let guardado = [];
      try {
        const r = await storageGet("feriados_catalogo");
        guardado = r ? JSON.parse(r.value) : [];
      } catch {
        guardado = [];
      }
      const anioActual = new Date().getFullYear();
      const defaults = defaultFeriadosCatalogo([anioActual - 1, anioActual, anioActual + 1, anioActual + 2]);
      const existentes = new Set(guardado.map((f) => f.fecha));
      const fusionado = [...guardado, ...defaults.filter((f) => !existentes.has(f.fecha))].sort((a, b) =>
        a.fecha.localeCompare(b.fecha)
      );
      setFeriadosCatalogo(fusionado);
    })();
  }, []);

  async function saveFeriadosCatalogo(nuevoCatalogo) {
    let ok = false;
    try {
      ok = await guardarDatoProtegido("feriados_catalogo", JSON.stringify(nuevoCatalogo), verifiedPinRef.current);
    } catch {
      ok = false;
    }
    if (!ok) {
    ...
    setFeriadosCatalogo(nuevoCatalogo);
    setToast({ color: sage, text: "Catálogo de días feriados actualizado." });
  }

  function toggleFeriadoActivo(fecha) {
    const actualizado = feriadosCatalogoDraft.map((f) => (f.fecha === fecha ? { ...f, activo: !f.activo } : f));
    setFeriadosCatalogoDraft(actualizado);
  }

  function agregarFeriadoPersonalizado() {
    const fecha = nuevoFeriadoFecha;
    const nombre = nuevoFeriadoNombre.trim();
    if (!fecha || !nombre) return;
    if (feriadosCatalogoDraft.some((f) => f.fecha === fecha)) {
      setToast({ color: paprika, text: "Ya hay un feriado guardado en esa fecha." });
      return;
    }
    const actualizado = [...feriadosCatalogoDraft, { fecha, nombre, esLey: false, activo: true }].sort((a, b) =>
      a.fecha.localeCompare(b.fecha)
    );
    setFeriadosCatalogoDraft(actualizado);
  ...
  function quitarFeriadoPersonalizado(fecha) {
    setFeriadosCatalogoDraft((d) => d.filter((f) => f.fecha !== fecha));
  }
  ...
  const [ajusteTipo, setAjusteTipo] = useState(""); // 'bono' | 'propina' | 'quitar' (penalización — retira de la propina)
  ...
  async function aplicarCorreccionManual() {
    if (!ajusteEmpleadoId || !ajusteFecha || !ajusteMotivo.trim() || !ajusteTipo) return;
    ...
    const esBono = ajusteTipo === "bono";
    const esPropina = ajusteTipo === "propina";
    const esQuitar = ajusteTipo === "quitar";
    const logEntry = {
      id: uid("ajuste"),
      ...
      fecha: ajusteFecha,
      bono: esBono,
      propina: esBono || esPropina,
      penalizacion: esQuitar,
    ...
    // "bono" ya incluye derecho a propina (mismo criterio que el resto de la app); "quitar" es
    // la penalización — deja a la persona sin calificar ese día ni para bono ni para propina.
    const nuevoTier = esBono ? "bono" : esPropina ? "propina" : "ninguno";
    const updatedDay = [...dayRecords];
    updatedDay[idx] = {
      ...updatedDay[idx],
      punctuality: nuevoTier,
      correccionManual: { motivo: ajusteMotivo.trim(), penalizacion: esQuitar, aplicadoEn: new Date().toISOString() },
    };
    ...
    setToast(
      esQuitar
        ? { color: paprika, text: `${emp?.name || "Empleado"} quedó fuera de la propina ese día — ya se refleja en todos lados.` }
        : { color: sage, text: `Corrección aplicada a ${emp?.name || "empleado"} — ya se refleja en todos lados.` }
    );
  }
  ...
  function openPayroll(employeeId) {
    ...
    setPayrollDraft({
      ...
      enablePermisos: false,
      // Feriados va en true por default: si hay un feriado dentro del periodo, la ley obliga
      // a pagarlo (trabajado o no) — apagarlo es una decisión explícita, no lo contrario.
      enableFeriados: true,
      feriadosOverrides: {}, // { [fecha]: { personalizado: true, monto, motivo } }
  ...
  function requestUnlock(target) {
    ...
    if (unlockedSession) {
      ...
      if (target === "feriados_catalogo_editar") {
        setFeriadosCatalogoDraft(feriadosCatalogo);
        setShowFeriadosCatalogoEdit(true);
      }
      return;
    }
    if (!pinConfigured) {
  ...
  async function handleFile(e) {
    ...
    if (!file) return;
    try {
      const dataUrl = await compressImage(file, 480, 0.6, { watermark: true });
  ...
  async function confirmPunch() {
    ...
    try {
      ...
      const photoPath = punchModal.photo ? photoStoragePath(recordId) : null;
      ...
      const record = {
        ...
        photoUrl: photoPath ? getPhotoPublicUrl(photoPath) : null,
      ...
      // la foto se sube a Supabase Storage (con marca de agua ya dibujada en handleFile);
      // se guarda la URL pública en el propio registro para verse desde cualquier dispositivo.
      if (punchModal.photo && photoPath) {
        uploadPhotoRemote(photoPath, punchModal.photo).catch((err) => {
          console.error("No se pudo subir la foto a la base de datos:", err);
          setToast({ color: paprika, text: "El registro se guardó, pero la foto no se pudo subir." });
        });
      }
  ...
  if (payrollPrintView) {
    ...
    const mostrarPermisos = pDraft.enablePermisos && (pDraft.permisosFechas || []).length > 0;
    ...
    // igual que vacaciones: recibos viejos sin este detalle congelado lo recalculan con la
    // configuración actual como respaldo (asume "olvido" si tampoco tienen tipo guardado).
    const pPermisosDetalle =
      payrollPrintView.permisosDetalle ||
      (pDraft.permisosFechas || []).map((p) => ({
        fecha: p.fecha,
        tipo: p.tipo || "olvido",
        motivo: p.motivo || "",
        monto: computeVacacionDayPay(pEmp, p.fecha, pDraft, pWorked),
      }));
    // igual que vacaciones: si el recibo ya trae su detalle congelado, se usa tal cual; si es
    // uno viejo generado antes de este cambio, se recalcula con el catálogo/bitácora actuales.
    const pFeriadosDetalle =
      payrollPrintView.feriadosDetalle ||
      getFeriadosEnRango(pDraft.periodStart, pDraft.periodEnd, feriadosCatalogo).map((f) => {
        const override = pDraft.feriadosOverrides?.[f.fecha];
        return {
          fecha: f.fecha,
          nombre: f.nombre,
          trabajado: empleadoTrabajoEseDia(pEmp.id, f.fecha, recordsByDate),
          monto: computeFeriadoMonto(pEmp, f.fecha, pDraft, pWorked, recordsByDate),
          personalizado: !!override?.personalizado,
          motivo: override?.personalizado ? override?.motivo || "" : "",
        };
      });
    const mostrarFeriados = pDraft.enableFeriados && pFeriadosDetalle.length > 0;
        ...
        >
          ...
          <table className="recibo-table">
            ...
            <tbody>
              ...
              {mostrarPermisos && (
                <tr>
                  <td>Permiso especial ({pDraft.permisosFechas.length} días)</td>
                  <td className="amt">+{formatMoney(pTotals.permisos)}</td>
                </tr>
              )}
              {mostrarFeriados && (
                <tr>
                  <td>Días feriados ({pFeriadosDetalle.length})</td>
                  <td className="amt">+{formatMoney(pTotals.feriados)}</td>
                </tr>
              )}
              {mostrarHorasExtra &&
                ...
                <tbody>
                  {pDraft.tieneSeguroSocial && (
                    <>
                      <tr>
                        <td>ISR retenido</td>
                        <td className="amt">({formatMoney(pTotals.isrDeduccion)})</td>
                      </tr>
                      <tr>
                        <td>IMSS (trabajador)</td>
                        <td className="amt">({formatMoney(pTotals.imssDeduccion)})</td>
                      </tr>
                    </>
                  )}
                  ...
                  <tr className="total-row">
                    ...
                    <td className="amt">
                      ({formatMoney(pTotals.isrDeduccion + pTotals.imssDeduccion + pTotals.descuentos + pTotals.consumo)})
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          {pDraft.tieneSeguroSocial && (
            <p style={{ fontSize: "0.56rem", color: "#555", marginTop: "0.3rem", lineHeight: 1.4 }}>
              * ISR e IMSS calculados con las tablas vigentes 2026 (Nómina Formal) — cálculo aproximado, no
              sustituye la validación de tu contador.
            </p>
          )}
          ...
          {mostrarPermisos && (
            <div style={{ marginTop: "0.85rem", border: "1.5px dashed #111", borderRadius: 5, padding: "0.7rem 0.85rem", position: "relative" }}>
              <p style={{ fontSize: "0.68rem", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.02em", margin: 0 }}>
                Recibo individual de permiso especial
              </p>
              <p style={{ fontSize: "0.56rem", color: "#555", marginTop: "1px" }}>
                Comprobante independiente de los días de este periodo que no quedaron registrados en el
                Checador (olvido de checada o permiso autorizado), pero que sí se pagan como jornada
                completa.
              </p>
              ...
              {pDraft.rateType === "hora" && (
                <p style={{ fontSize: "0.52rem", color: "#666", marginTop: "2px" }}>
                  El monto de cada día se calculó según el horario asignado para ese día de la semana, no
                  la tarifa por hora aplicada a una sola hora.
                </p>
              )}
              ...
              <table className="recibo-table">
                ...
                <tbody>
                  {pPermisosDetalle.map(({ fecha, tipo, motivo, monto }) => (
                    <tr key={fecha}>
                      <td>{formatDateLabel(fecha, today)}</td>
                      <td>{fecha}</td>
                      <td>
                        {tipo === "permiso" ? "Permiso autorizado" : "Olvidó checar"}
                        {motivo ? ` — ${motivo}` : ""}
                      </td>
                      <td className="amt">{formatMoney(monto)}</td>
                    </tr>
                  ...
                  <tr className="total-row">
                    <td colSpan={3}>Total días de permiso especial ({pDraft.permisosFechas.length})</td>
                    <td className="amt">{formatMoney(pTotals.permisos)}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <p style={{ fontSize: "0.62rem", lineHeight: 1.45, color: "#111", marginTop: "0.5rem", borderLeft: "3px solid #111", paddingLeft: "0.55rem" }}>
                Quien suscribe, <strong>{pEmp.name}</strong>, reconoce que los días señalados arriba no
                quedaron registrados en el Checador (por olvido o por permiso autorizado por el negocio) y
                recibe a su entera satisfacción el pago correspondiente a cada uno de esos días, como si
                hubiera cubierto la jornada completa.
              </p>
              <p style={{ fontSize: "0.56rem", color: "#555", marginTop: "0.3rem" }}>
                El monto ya está incluido en el neto a pagar de arriba — este apartado es el comprobante
                individual.
          ...
          {mostrarFeriados && (
            <div style={{ marginTop: "0.85rem", border: "1.5px dashed #111", borderRadius: 5, padding: "0.7rem 0.85rem", position: "relative" }}>
              <p style={{ fontSize: "0.68rem", fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.02em", margin: 0 }}>
                Comprobante de días feriados
              </p>
              <p style={{ fontSize: "0.56rem", color: "#555", marginTop: "1px" }}>
                Días feriados del catálogo dentro de este periodo, conforme a los artículos 74 y 75 de la Ley
                Federal del Trabajo: el día no trabajado se paga completo; el día trabajado se paga triple
                (pago base ya incluido arriba + el doble que se detalla aquí).
              </p>
              ...
              <table className="recibo-table">
                ...
                <tbody>
                  {pFeriadosDetalle.map((f) => (
                    <tr key={f.fecha}>
                      <td>
                        {f.nombre}
                        {f.personalizado && f.motivo && (
                          <div style={{ fontSize: "0.5rem", color: "#666", marginTop: "1px" }}>
                            Monto personalizado — {f.motivo}
                          </div>
                        )}
                      </td>
                      <td>{f.fecha}</td>
                      <td>{f.trabajado ? "Sí" : "No"}</td>
                      <td className="amt">{formatMoney(f.monto)}</td>
                    </tr>
                  ))}
                  <tr className="total-row">
                    <td colSpan={3}>Total días feriados ({pFeriadosDetalle.length})</td>
                    <td className="amt">{formatMoney(pTotals.feriados)}</td>
                  </tr>
                </tbody>
              </table>
  ...
  return (
    ...
    >
      ...
      {tab === "checador" && (
        <div className="flex-1 px-5 py-5 flex flex-col gap-5">
          <div
            ...
            style={{ padding: 3, background: brass, boxShadow: `0 6px 18px ${ink}77` }}
          >
            <div
              className="rounded-sm px-6 py-5 flex flex-col items-center justify-center"
              style={{ background: paper }}
            >
              {(() => {
                const horaTexto = horaActual.toLocaleTimeString("es-MX", {
                  hour: "2-digit",
                  minute: "2-digit",
                  second: "2-digit",
                });
                const fechaTexto = horaActual.toLocaleDateString("es-MX", {
                  weekday: "long",
                  day: "numeric",
                  month: "long",
                });
                ...
                return (
                  <>
                    <div className="flex items-center gap-2 mb-2 w-full">
                      <span className="h-px flex-1" style={{ background: steel + "55" }} />
                      <span
                        className="text-[9px] font-bold uppercase whitespace-nowrap"
                        style={{ color: steel, letterSpacing: "0.2em" }}
                      >
                        Hora actual
                      </span>
                      <span className="h-px flex-1" style={{ background: steel + "55" }} />
                    ...
                    <div
                      ...
                      style={{ color: ink, fontSize: "3rem", letterSpacing: "0.03em", lineHeight: 1 }}
                    >
                      {horaTexto}
                    </div>
                    <div
                ...
                return (
                  <div key={date}>
                    ...
                    <div className="flex flex-col gap-2">
                      {dayRecords.map((r) => {
                        const photoUrl = r.photoUrl || (r.hasPhoto ? photoCache[r.id] : null);
                        const meta =
                          ...
                          >
                            {photoUrl ? (
                              <img
                                ...
                                alt={r.employeeName}
                                onClick={() =>
                                  setPhotoViewer({ url: photoUrl, employeeName: r.employeeName, type: r.type, time: r.time })
                                }
        ...
        <div className="flex-1 px-5 py-5 flex flex-col min-h-0 overflow-y-auto">
          {!payrollSelectedEmployeeId ? (
            <>
              ...
              {/* Catálogo de días feriados: fechas que Nómina detecta solo dentro del periodo de
                  cada empleado (ver PayrollPanel → "Días feriados"). Visible sin PIN; editarlo
                  (activar/desactivar una fecha, o agregar una propia) sí lo pide. */}
              <div className="rounded-sm p-4" style={{ background: paper }}>
                <div className="flex items-center justify-between mb-1">
                  <div className="text-[11px] font-bold uppercase" style={{ color: ink, letterSpacing: "0.05em" }}>
                    Días feriados
                  </div>
                  {!showFeriadosCatalogoEdit && (
                    <button
                      onClick={() => requestUnlock("feriados_catalogo_editar")}
                      className="flex items-center gap-1 text-[10px] font-bold uppercase px-2.5 py-1.5 rounded-sm flex-shrink-0"
                      style={{ border: `1px solid ${ink}33`, color: ink }}
                    >
                      <Lock size={11} /> Editar
                ...
                {!showFeriadosCatalogoEdit ? (
                  <>
                    <p className="text-[11px] mt-1" style={{ color: ink + "88" }}>
                      {feriadosCatalogo.filter((f) => f.activo).length} fecha(s) activa(s) de{" "}
                      {feriadosCatalogo.length} en el catálogo.
                    </p>
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {feriadosCatalogo
                        .filter((f) => f.activo && f.fecha >= localDateKey())
                        .slice(0, 6)
                        .map((f) => (
                          <span
                            key={f.fecha}
                            className="text-[10px] font-bold px-2 py-1 rounded-sm"
                            style={{ background: sage + "14", color: ink }}
                          >
                            {f.fecha.slice(8, 10)}/{f.fecha.slice(5, 7)} · {f.nombre}
                          </span>
                        ))}
                    </div>
                  </>
                  ...
                  <div className="flex flex-col gap-1.5 mt-2">
                      ...
                      {feriadosCatalogoDraft.map((f) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <MiniToggle
                              checked={f.activo}
                              onChange={() => toggleFeriadoActivo(f.fecha)}
                              colors={{ ink, sage, steel }}
                            />
                            {!f.esLey && (
                              <button onClick={() => quitarFeriadoPersonalizado(f.fecha)} className="p-0.5">
                                <Trash2 size={13} color={ink + "55"} />
                              </button>
                            )}
                          </div>
                        </div>
                    ...
                    <p className="text-[10px] mt-1" style={{ color: ink + "66" }}>
                      Agregar una fecha propia (ej. un festivo local o bancario que decidas pagar):
                    </p>
                    <div className="flex items-center gap-1.5">
                      <input
                        type="date"
                      ...
                      />
                      <button
                        onClick={agregarFeriadoPersonalizado}
                        disabled={!nuevoFeriadoFecha || !nuevoFeriadoNombre.trim()}
                        className="p-1.5 rounded-sm flex-shrink-0 disabled:opacity-40"
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <button
                        onClick={() => saveFeriadosCatalogo(feriadosCatalogoDraft)}
                        className="text-[10px] font-bold uppercase px-3 py-1.5 rounded-sm"
                        style={{ background: sage, color: paper }}
                      >
                        Guardar
                ...
                <p className="text-[10px] mt-2" style={{ color: ink + "66" }}>
                  Cualquiera puede ver este catálogo; solo con la clave se puede cambiar. Los feriados de ley
                  vienen precargados; puedes desactivar cualquiera (sin borrarlo) o agregar los tuyos.
                </p>
              </div>
            ...
            <PayrollPanel
              ...
              feriadosCatalogo={feriadosCatalogo}
              onBack={closePayroll}
                      ...
                      {lista.map((p) => (
                        ...
                        >
                          <div>
                            ...
                            {p.motivosCorreccion && p.motivosCorreccion.length > 0 && (
                              <div className="mt-1 flex flex-col gap-0.5">
                                {p.motivosCorreccion.map((c, i) => (
                                  <div
                                    key={i}
                                    className="text-[10px] font-semibold"
                                    style={{ color: c.penalizacion ? paprika : sage }}
                                  >
                                    💬 {c.penalizacion ? "Penalización" : "Ajuste"}
                                    {p.motivosCorreccion.length > 1 ? ` (${formatDateLabel(c.fecha, today)})` : ""}
                                    {c.motivo ? `: ${c.motivo}` : ""}
                                  </div>
                                ))}
                              </div>
          ...
          <div className="rounded-sm p-4" style={{ background: paper }}>
            ...
            {!showAjusteManual ? (
              <p className="text-[10px] mt-1" style={{ color: ink + "66" }}>
                ...
                etc.) le impidió calificar para bono o propina, o para retirar a alguien del conteo
                de propinas por una falta (con motivo). Requiere la clave.
              </p>
            ) : (
              <div className="mt-2 flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="flex items-center gap-2 text-xs" style={{ color: ink }}>
                    <input
                      type="radio"
                      name="ajusteTipo"
                      checked={ajusteTipo === "bono"}
                      onChange={() => setAjusteTipo("bono")}
                    />
                    Bono de puntualidad (incluye propina)
                  </label>
                  <label className="flex items-center gap-2 text-xs" style={{ color: ink }}>
                    <input
                      type="radio"
                      name="ajusteTipo"
                      checked={ajusteTipo === "propina"}
                      onChange={() => setAjusteTipo("propina")}
                    />
                    Solo propina
                  </label>
                  <label className="flex items-center gap-2 text-xs" style={{ color: paprika }}>
                    <input
                      type="radio"
                      name="ajusteTipo"
                      checked={ajusteTipo === "quitar"}
                      onChange={() => setAjusteTipo("quitar")}
                    />
                    Quitar de la propina (penalización)
                  </label>
                </div>
                ...
                <input
                  ...
                  placeholder={
                    ajusteTipo === "quitar"
                      ? "Motivo de la penalización (obligatorio)"
                      : "Motivo del ajuste (obligatorio, ej. falla al checar)"
                  }
                  className="w-full px-2 py-1.5 rounded-sm text-xs outline-none"
                  style={{ border: `1px solid ${ink}33`, background: "#fff", color: ink }}
                />
                <button
                  ...
                  disabled={!ajusteEmpleadoId || !ajusteFecha || !ajusteMotivo.trim() || !ajusteTipo}
                  className="flex items-center justify-center gap-2 py-2 rounded-sm font-bold text-xs uppercase disabled:opacity-40"
                  style={{
                    background: ajusteTipo === "quitar" ? paprika : brass,
                    color: ajusteTipo === "quitar" ? "#fff" : ink,
                  }}
                >
                  {ajusteTipo === "quitar" ? "Aplicar penalización" : "Aplicar corrección"}
                </button>
                  ...
                  <div className="flex flex-col gap-1.5 mt-1 max-h-40 overflow-y-auto">
                    {ajustesLog.slice(0, 10).map((a) => (
                      <div
                        ...
                        style={{ background: a.penalizacion ? paprika + "1c" : brass + "1c" }}
                      >
                        <div className="text-[10px]" style={{ color: ink }}>
                          <strong>{a.employeeName}</strong> · {formatDateLabel(a.fecha, today)} ·{" "}
                          {a.penalizacion ? "Quitado de propina (penalización)" : a.bono ? "Bono + propina" : "Propina"}
                        </div>
                          ...
                          <div className="flex flex-col gap-1">
                            {p.reparto.map((item, i) => (
                              <div key={i} className="flex flex-col text-[10px]">
                                ...
                                {item.motivosCorreccion && item.motivosCorreccion.length > 0 && (
                                  <div className="flex flex-col gap-0.5 mt-0.5">
                                    {item.motivosCorreccion.map((c, j) => (
                                      <div
                                        key={j}
                                        className="text-[10px] font-semibold"
                                        style={{ color: c.penalizacion ? paprika : sage }}
                                      >
                                        💬 {c.penalizacion ? "Penalización" : "Ajuste"}
                                        {item.motivosCorreccion.length > 1 ? ` (${formatDateLabel(c.fecha, today)})` : ""}
                                        {c.motivo ? `: ${c.motivo}` : ""}
                                      </div>
                                    ))}
                                  </div>
      ...
      {photoViewer && (
        ...
        >
          <div className="w-full max-w-sm rounded-sm p-3" style={{ background: paper }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <div className="text-xs font-bold" style={{ color: ink }}>
                {photoViewer.employeeName} ·{" "}
                <span style={{ color: photoViewer.type === "entrada" ? paprika : sage }}>
                  {photoViewer.type} · {formatTime(photoViewer.time)}
                </span>
              </div>
            ...
            <img
              src={photoViewer.url}
              alt={photoViewer.employeeName}
              className="w-full rounded-sm object-contain"
              style={{ maxHeight: "70vh", background: charcoal }}
            />
          </div>
        </div>
      )}
